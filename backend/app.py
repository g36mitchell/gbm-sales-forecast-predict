...
# Define an endpoint for batch sales forecast (POST request)
...
def predict_sales_forecast_batch():
    ...
    and returns the predicted sales forecast as a dictionary in the JSON response.
    """
    ...
    predicted_forecast = model.predict(input_data).tolist()

    # Return the predictions a JSON response
    return jsonify({"predictions": predicted_forecast})
